 ...
 * with the values sent for address, weight, and side1.
 * Then it is to call the set_measurements method
 * with the values sent for side1, side2, and side3.
 * The set_measurements method throws an out_of_range exception.
 *
 ...
 * @param side3 - one dimension of the carton
 */
Carton::Carton(Address address, double weight, double side1, double side2, double side3)
: ShippingItem(address, weight, side1)
{
    try {
        set_measurements(side1, side2, side3);
    }
    catch (out_of_range){throw out_of_range ("All measurements must be greater than zero");}
}
...
/**
 * @brief This method will output the word Carton
 * followed by weight, length, width, and height using :, lbs., and x.
 *
 * @param out - an output file stream
 */
void Carton::Display(std::ostream &out) const
{
    out << std::fixed << setprecision(1) << "Carton: " << get_weight() << " lbs. " << length_ << " x "
